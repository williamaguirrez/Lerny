...
export const store = new Vuex.Store({
    state: {
        ...
        guardarDatabase(id, usuario, email, foto){
            ...
            firebase.database().ref('/perfiles/'+ id).once('value', data => {
                if (data.val() == null){
                    firebase.database().ref('/perfiles/' + id).set({
                        UID: id,
                        nombre: usuario,
                        rol: '',
                        telefono: '',
        ...
        guardarPerfil(id, nombre, rol, telefono, sitioweb, cumpleanos, descripcion, habilidades, genero, niveleducativo){
            firebase.database().ref('/perfiles/' + id).update({
                UID: id, nombre: nombre, rol: rol, telefono: telefono, sitioweb: sitioweb, cumpleanos: cumpleanos,
                descripcion: descripcion, habilidades: habilidades, genero: genero, niveleducativo: niveleducativo,
            }).then(result =>{
                this.mensajeExito('Gracias por Actualizar','Guardado Correctamente');
            }).catch(err => {
